...
import { PLAYER_OPTIONS } from '../../game/config';
...
export const PlayerSelectionScreen: React.FC<PlayerSelectionScreenProps> = ({ onStartGame }) => {
            ...
            <div className="flex items-center ml-6">
              <input
                ...
                className={`h-5 w-5 rounded border-gray-300 text-orange-600 focus:ring-orange-500 ${
                  !skipDraft ? 'opacity-50 cursor-not-allowed' : 'bg-white cursor-pointer'
                }`}
              />
              <label
                htmlFor="skip-campaign-checkbox"
                className={`ml-3 ${
                  !skipDraft ? 'text-slate-400 cursor-not-allowed' : 'text-slate-600 cursor-pointer'
                }`}
              >
