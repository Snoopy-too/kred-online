...
export const PerfectTileModal: React.FC<PerfectTileModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      logDiag({ category: 'dialog', event_type: 'DIALOG_OPENED', payload: { dialog_type: 'PerfectTile' } });
    }
  }, [isOpen, logDiag]);
  ...
  return (
    ...
    >
      <div className="bg-gray-800 border-2 border-green-500 rounded-xl text-center shadow-2xl max-w-md w-full p-6 sm:p-8">
        ...
        <button
          onClick={() => {
            logDiag({ category: 'dialog', event_type: 'DIALOG_OK', payload: { dialog_type: 'PerfectTile' } });
            onContinue();
          }}
...
export const FinishTurnConfirmModal: React.FC<FinishTurnConfirmModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      logDiag({ category: 'dialog', event_type: 'DIALOG_OPENED', payload: { dialog_type: 'FinishTurnConfirm', remainingKredcoin } });
    }
    ...
    >
      <div className="bg-gray-800 border-2 border-yellow-500 rounded-xl text-center shadow-2xl max-w-md w-full p-6 sm:p-8">
        ...
        <div className="flex gap-4 justify-center">
          <button
            onClick={() => {
              logDiag({ category: 'dialog', event_type: 'DIALOG_CANCEL', payload: { dialog_type: 'FinishTurnConfirm' } });
              onCancel();
            }}
          ...
          <button
            onClick={() => {
              logDiag({ category: 'dialog', event_type: 'DIALOG_OK', payload: { dialog_type: 'FinishTurnConfirm' } });
              onConfirm();
            }}
