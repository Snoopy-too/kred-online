...
import type { PlayerRostrum, RostrumSupport } from "../types/rostrum";
import { getPlayerIdFromLocationId } from "../game/locations";
...
export function getRostrumSupportRule(
  ...
  return playerRules.rostrums.find((r: any) => r.rostrum === rostrumId) || null;
}
...
export function countPiecesInPlayerRostrums(
  ...
  const rostrumIds = playerRules.rostrums.map((r: any) => r.rostrum);
  return pieces.filter(
